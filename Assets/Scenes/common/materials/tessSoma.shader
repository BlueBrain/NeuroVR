Shader "JJ/tessSoma" 
{
    Properties{
        ...
        _minLevel("Min Level", Float) = 4
        _maxLevel("Max Level", Float) = 20
    ...
    SubShader{
        Pass{
            ...
            TessAttributes vert(VertAttributes input)
            {
                ...
                float level =  (1 - clamp((distance(center, _WorldSpaceCameraPos.xyz)) * _maxDistanceInv, 0, 1))* (maxLevel-minLevel) + minLevel;
            ...
            FragAttributes domain(
                TessFactors factors, 
                OutputPatch<TessAttributes, 3> patch, float3 uv: SV_DomainLocation)
            {
                UNITY_SETUP_INSTANCE_ID(patch[0]);
                float3 center = patch[0].center * uv.x + patch[1].center * uv.y + patch[2].center * uv.z;
                float3 normal = normalize(patch[0].normal * uv.x + patch[1].normal * uv.y + patch[2].normal * uv.z);
                float radius = patch[0].levelRadius.y;  
